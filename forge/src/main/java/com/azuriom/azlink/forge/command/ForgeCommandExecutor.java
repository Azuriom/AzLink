...
public class ForgeCommandExecutor<S extends CommandSourceStack>
        extends AzLinkCommand implements Command<S>, SuggestionProvider<S> {

    public ForgeCommandExecutor(AzLinkPlugin plugin) {
        super(plugin);
    }
    ...
    public int run(CommandContext<S> context) {
        try {
            ...
            ForgeCommandSource source = new ForgeCommandSource(context.getSource());
