...
public class NeoForgeCommandExecutor<S extends CommandSourceStack>
        extends AzLinkCommand implements Command<S>, SuggestionProvider<S> {

    public NeoForgeCommandExecutor(AzLinkPlugin plugin) {
        super(plugin);
    }
    ...
    public int run(CommandContext<S> context) {
        try {
            ...
            NeoForgeCommandSource source = new NeoForgeCommandSource(context.getSource());
